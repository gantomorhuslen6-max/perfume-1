...
export default function ProductCard({ perfume }: ProductCardProps) {
  ...
  const quantityInCart = getItemQuantity(perfume.id);
  ...
  return (
    <div className="group relative flex flex-col rounded-lg overflow-hidden">
      <Link href={`/perfumes/${perfume.id}`} className="flex flex-col flex-1">
        {/* Badges */}
      ...
      <button 
        ...
        className="border border-black text-black px-4 py-2 text-sm hover:bg-black hover:text-white transition-colors duration-200 uppercase tracking-wide"
      >
        {quantityInCart > 0 ? `Сагсанд нэмэх (${quantityInCart})` : 'Сагсанд нэмэх'}
      </button>
    </div>
  );
}
